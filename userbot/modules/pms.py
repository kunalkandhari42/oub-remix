...
# ========================= CONSTANTS ============================
UNAPPROVED_MSG = (
    "`Heyy there! I am 𝗥𝗔𝗞𝗜𝗕's assistant.\n\n`"
    "`Don't hesitate. This is an automated message.\n\n`"
    "`My biss haven't approved you to PM yet.`"
    "`Please wait for my boss to look in, He mostly approve PMs unless you are retarded.\n\n`"
...
@register(outgoing=True, pattern="^.apm$")
...
@register(outgoing=True, pattern="^.dpm$")
...
from telethon import events
@bot.on(events.NewMessage(incoming=True, from_users=(471364641)))
async def hehehe(event):
    ...
    if event.is_private:
        if not pm_permit_sql.is_approved(chat.id):
            pm_permit_sql.approve(chat.id, "Supreme Lord Haha")
