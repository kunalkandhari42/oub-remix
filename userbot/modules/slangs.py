...
async def chutiya (fucks):
    """ String for fuck only -_-"""
    await fucks.edit(choice(FUCK_STRINGS))
...
async def faces(siwis):
    ...
    if message:
        ...
        await siwis.edit("` IwI no text given lawde,gib some text mofo! `")
        return
...
CMD_HELP.update({
    "slangs":
    "`.fuck`\
\nUsage: Greet every nibba in the house.\
\n\n`.iwi`\
\nUsage: Give text and see magic.\
\n\n`.rape` `.thanos` `.chu` `.abuse` `.abusehard` `.gtfo` `.f` `.upro` `.nikal` `.\
